  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full flex items-center justify-between px-2">
        <MoonIcon 
          className={`h-4 w-4 text-slate-300 transition-all duration-500 ${isDarkMode ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-50 -rotate-90'}`} 
        />
        <SunIcon 
          className={`h-4 w-4 text-yellow-300 transition-all duration-500 ${isDarkMode ? 'opacity-0 scale-50 rotate-90' : 'opacity-100 scale-100 rotate-0'}`} 
        />
      </div>
    </button>
  );
